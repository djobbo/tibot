  ...
  switch(args[0]) {
    case 'tibadd':
      ...
      try {
        ...
        let res = args.reduce((acc, arg, i) => i >= 2 ? `${acc} ${arg}`: acc, '');
        await db.doc(`commands/${command}`).set({
          res: res
        });
        msg.reply(`La commande "${command} => ${res}" a bien été crée!`);
      }
      catch(e) {
        console.error(e);
        return;
    ...
    case 'tibrm':
      ...
      try {
        ...
        msg.reply(`La commande "${command} => ${commandDoc.data().res}" a bien été supprimée!`);
      }
      catch(e) {
        console.error(e);
        return;
    ...
    case 'tibcmd':
      ...
      let cmdStr = cmdDocs.docs.map(cmd => `${cmd.id} => ${cmd.data().res}`).join('\n');
    ...
    case 'tibhelp':
      let helpEmbed = new Discord.RichEmbed()
        ...
        .setURL('https://dvmm.dev/')
        .setDescription('Tibis a votre service!')
        .addField('Créer/Modifier une commande', '`tibadd [commande] [réponse]`')
